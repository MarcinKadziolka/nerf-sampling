...
class SphereConcatNeRF(nn.Module):
    def __init__(
        self, D=8, W=256, input_ch=3,
        input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False
    ):
        """
        Create a model with concatenated inputs.

        The model uses an input consisting of both points and
        camera angles. Respective tensors are concatenated into
        a single input.
        This is also the case for skip layers.
        """
        super(SphereConcatNeRF, self).__init__()
        self.D = D
        self.W = W
        ...
        self.pts_linears = nn.ModuleList(
            [nn.Linear(input_dim, W)] + [nn.Linear(W, W)
