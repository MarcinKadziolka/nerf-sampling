...
class Line:
    def __init__(self, origin: torch.Tensor(1, 3), direction: torch.Tensor(1, 3)):
        self.origin = origin
        self.direction = normalize(direction)

    # equation to solve: d^2 + bd + c = 0
    # where solutions d are the distances from the line origin to the intersection points
    def find_intersection_points_with_sphere(self, sphere: Sphere) -> [torch.Tensor(1, 3)]:
        ...
        intersection_points = [torch.Tensor(self.origin + d * self.direction) for d in equation_solutions]
        return intersection_points

    def select_closest_point_to_origin(self, points: [torch.Tensor(1, 3)]):
